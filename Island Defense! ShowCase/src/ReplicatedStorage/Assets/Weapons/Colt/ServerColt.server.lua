		...
		local Humanoid = HitPart.Parent:FindFirstChild("Humanoid") --Is there a humanoid?
			...
			if tagged then
				if HitPart.Name == "Handle" or HitPart.Name == "Head" then
					Humanoid.Health = Humanoid.Health-180 --Damage.
					tagHumanoid(Humanoid, Player)	
					--wait(2)
					--untagHumanoid(Humanoid)
				else
					Humanoid.Health = Humanoid.Health-120
					tagHumanoid(Humanoid, Player)
					--wait(2)
					--untagHumanoid(Humanoid)
				end
...
Tool.Unequipped:Connect(function()
	...
	if Player and Character then
		...
		if Torso and Tool == nil then
			local Model = Instance.new('Model')
			Model.Name = 'Fake'..script.Parent.Name
			Model.Parent = Character
			
			FakeHandle = script.Parent.Colt:Clone()
			FakeHandle.CanCollide = false
			FakeHandle.Name = Model.Name
			FakeHandle.Parent = Model
			
			local Weld = Instance.new('Weld')
			Weld.Name = 'BackWeld'
			Weld.Part0 = Torso
			Weld.Part1 = FakeHandle
			Weld.C0 = CFrame.new(-1.1, -1.43, 0)
			Weld.C0 = Weld.C0 * CFrame.fromEulerAnglesXYZ(5, math.rad(190), -0)
